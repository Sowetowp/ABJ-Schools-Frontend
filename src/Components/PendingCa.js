...
import { get_assignments, get_ca, rec_assignment, rec_ca } from '../Redux/Actions/StudentAction'
...
const PendingCa = () => {
    ...
    const getCa = useSelector((state) => state.getCa)
	const { cas, loading } = getCa
    const recCa = useSelector((state) => state.recCa)
	const { ca, loading2 } = recCa
    useEffect(() => {
  ...
  return (
    <>
        ...
        <main className='container-fluid'>
            <h1 className='passh1'>Pending CA</h1>
            <p className='passp1'>CA / Pending CA</p>
            <div id="accordion" className='mt-5'>
                {cas && cas.length > 0 && cas.map((assign, index)=>
                <div className="card">
