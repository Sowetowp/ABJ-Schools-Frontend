...
import { admin_get_all_parents, admin_search_parents, delete_single_parent } from '../Redux/Actions/AdminAction'
...
const ParentList = () => {
    ...
    const adminSearchParents = useSelector((state) => state.adminSearchParents)
	const { searched } = adminSearchParents
    const [results, setResults] = useState([])
    
    const handleChange = (value) => {
        setInput(value);
        dispatch(admin_search_parents()).then(() => {
            if(searched){
    ...
    const adminGetAllParents = useSelector((state) => state.adminGetAllParents)
	const { loading, parents } = adminGetAllParents
    const parentDelete = useSelector((state) => state.parentDelete)
	const { success } = parentDelete
    
    const dispatch = useDispatch()
    useEffect(() => {
        dispatch(admin_get_all_parents(1))
    }, [])
    const handleItemClick = (item) => {
        dispatch(admin_get_all_parents(item))
    }

    const handlePrev = () => {
        if (parents.currentPage > 1) {
          dispatch(admin_get_all_parents(parents.currentPage - 1));
        }
    };

    const handleNext = () => {
        if (parents.currentPage !== parents.totalPages) {
          dispatch(admin_get_all_parents(parents.currentPage + 1));
        }
    };
    ...
    const deleteUserHandler = (e, id) => {
		e.preventDefault()
		dispatch(delete_single_parent(id)) 
    }
    useEffect(() => {
        if(success){
            dispatch(admin_get_all_parents(parents && parents.currentPage));
        }
    }, [success])
                                ...
                                <div className='shoiwtrsb2 p-3 my-5'>
                                    ...
                                    <div className="table-responsive shoiwtrsb">
                                        ...
                                        <tbody>
                                            ...
                                            :
                                            parents && parents.teachers && parents.teachers.map((teacher, index) =>
                                            <tr>
                                                <td>{index + 1}</td>
                                                <td>{teacher.firstName}</td>
                                                <td>{teacher.lastName}</td>
                                    ...
                                    {parents && parents.length > 0 && results.length < 1 && parents.totalPages &&
                                    <div className=''>
                                        <ul className="pagination justify-content-center pagination-sm newpage py-5">
                                            <li className={`page-item ${parents.currentPage <= 1? "disabled" : ""}`}><a className="page-link" onClick={handlePrev}>Previous</a></li>
                                            {Array.from({ length: parents && parents.totalPages }, (_, index) => index + 1).map((item) => (
                                                <li className="page-item" key={item}><a className="page-link" onClick={() => handleItemClick(item)}>{item}</a></li>
                                            ))}
                                            <li className={`page-item ${parents.currentPage === parents.totalPages? "disabled" : ""}`}><a className="page-link" onClick={handleNext}>Next</a></li>
                                        </ul>
                                    </div>}
                                </div>
                            </fieldset>
